  ...
  <el-card style="max-width: 100%">
    <template #header>
      <div class="card-header">
        <span>就诊人管理</span>
        <el-button @click="addUser" type="primary" :icon="User">添加就诊人</el-button>
      </div>
    </template>
    <!-- 就诊人管理模块展示就诊人信息 -->
    <div class="visitor" v-if="scene == 0">
      <Visitor @changeScene="changeScene" class="item" v-for="(user, index) in userArr" :key="user.id" :user="user"
        :index="index" @removeUser="removeUser" />
    </div>
    <!-- 添加就诊人或者修改已有的就诊人信息结构 -->
    <div class="form" v-if="scene == 1">
      ...
      <el-form style="width: 60%; margin: 10px auto;">
        <el-form-item label="用户姓名">
          <el-input placeholder="请你输入用户的姓名" v-model="userParams.name"></el-input>
        </el-form-item>
        ...
        <el-form-item label="出生日期">
          <el-date-picker type="date" placeholder="请你选择日期" value-format="YYYY-MM-DD" v-model="userParams.birthdate" />
        </el-form-item>
        <el-form-item label="手机号码">
          <el-input placeholder="请你输入用户的手机号码" v-model="userParams.phone"></el-input>
        </el-form-item>
      </el-form>
      ...
      <el-form style="width: 60%; margin: 10px auto;">
        ...
        <el-form-item label="详细地址">
          <el-input placeholder="请你输入用户的详细地址" v-model="userParams.address"></el-input>
        </el-form-item>
      </el-form>
      <el-divider content-position="left">联系人信息（选填）</el-divider>
      <el-form style="width: 60%; margin: 10px auto;" label-width="80">
        <el-form-item label="用户姓名">
          <el-input placeholder="请你输入用户的姓名" v-model="userParams.contactsName"></el-input>
        </el-form-item>
        <el-form-item label="证件类型">
        ...
        <el-form-item label="手机号码">
          <el-input placeholder="请你输入用户的手机号码" v-model="userParams.contactsPhone"></el-input>
        </el-form-item>
        <el-form-item>
...
import { reqCertationType, reqCity, reqAddOrUpdataUser } from '@/api/user/index'
import type { UserResponseData, UserArr } from '@/api/hospital/type'
import type { CertationTypeResponseData, CertationArr, AddOrUpdataUser } from '@/api/user/type'
import {
  User
} from '@element-plus/icons-vue'
import { onMounted, ref, reactive,watch } from 'vue'
//@ts-ignore
import { tryCall } from 'element-plus'
...
onMounted(() => {
  ...
  // 获取证件的类型的数据
  getCertationType()
  // 判断当前的路由组建是不是从挂号组建来的[挂号组建来他的路径上是携带query参数type=add]
